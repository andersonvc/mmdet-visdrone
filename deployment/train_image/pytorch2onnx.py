...
def pytorch2onnx(
    ...
    if dynamic_export:
        dynamic_axes = {
            input_name: {0: "batch", 2: "width", 3: "height"},
    ...
    if verify:
        ...
        with torch.no_grad():
            pytorch_results = model(
                img_list, img_metas=img_meta_list, return_loss=False, rescale=True
            )[0]
        ...
        # get onnx output
        onnx_results = onnx_model(img_list, img_metas=img_meta_list, return_loss=False)[
            0
        ]
...
def parse_args():
    ...
    parser.add_argument(
        "--show", action="store_true", help="Show onnx graph and detection outputs"
    )
